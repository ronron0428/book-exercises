...
year_2017_salaries <- runif(100,40000,50000)
...
salaries <- data.frame(employees,year_2017_salaries,annual_salary_adjustments, stringAsFactors = FALSE)

# Add a column to the `salaries` data frame that represents each person's
# salary in 2018 (e.g., with the salary adjustment added in).
salaries$year_2018_salaries <- salaries$year_2017_salaries + salaries$annual_salary_adjustments

# Add a column to the `salaries` data frame that has a value of `TRUE` if the 
# person got a raise (their salary went up)
salaries$raise <- salaries$year_2018_salaries > salaries$year_2017_salaries
